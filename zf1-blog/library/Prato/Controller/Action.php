...
/*class Blog_Controller_Action extends Zend_Controller_Action {
	...
	public function init() {  //vai ser executado antes de cada action e está no final do construtor

		$auth = Zend_Auth::getInstance();   //pega a instância

		if ($this ->getParam('controller') != 'login') //se a url não está no controller login, se for diferente
	    {

		if (!$auth ->hasIdentity())  //se o usuário não está autenticado
		{
			$this ->_helper ->redirector ->gotoSimpleAndExit('index', 'login');  //redireciona para o login
		}
	  }
	}
